...
public sealed class MatchContextStrategy : IMatchContextStrategy
{
    ...
    public async void ReceivedOrder(MatchingEngine order, CancellationToken cancellationToken)
    {
        ...
        bool result = await this.MatchOrderAsync(order, cancellationToken);
        if (!result) throw new Exception($"Deu algum erro no matching, favor verificar. {JsonSerializer.Serialize(order)} ");
    }
