...
public class TradeEngineOutboxApp : IConsumer<EnvelopeOutbox<Trade>>
{
    private readonly ILogger<DropCopyOutboxApp> _logger;
    private PushSocket _sender = null!;
    private readonly ConcurrentQueue<EnvelopeOutbox<Trade>> _queueEnvelopeOutbox = null!;
    ...
    public TradeEngineOutboxApp(ILogger<DropCopyOutboxApp> logger, IOptions<ConnectionZmq> options)
    {
        _logger = logger;
        _queueEnvelopeOutbox = new ConcurrentQueue<EnvelopeOutbox<Trade>>();
    ...
    public Task Consume(ConsumeContext<EnvelopeOutbox<Trade>> context)
    {
    ...
    public void SenderActivity(CancellationToken cancellationToken)
    {
        ...
        {
            while (_queueEnvelopeOutbox.TryDequeue(out EnvelopeOutbox<Trade> envelope))
            {
                switch (envelope.ActivityOutbox.Activity)
                {
                    case OutboxActivities.MatchingToDropCopySent:
                        {
                            using (_sender = new PushSocket("@" + _config.MatchingToTradeEngine))
                            {
                                var message = envelope.Body.SerializeToByteArrayProtobuf<Trade>();
